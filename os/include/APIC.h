...
struct APIC_INFO {
	/* 记录APIC设备相关信息 */
	unsigned char xapic;	/* 是否支持xapic(0 - 不支持; 1 - 支持) */
	unsigned char x2apic;	/* 是否支持x2apic(0 - 不支持; 1 - 支持) */
} apic_info;

struct local_APIC_map {
	/* local APIC相关寄存器的一些地址映射结构 */
	mem_addr32 physical_address;						/* 物理地址 */
	mem_addr8 *virtual_index_address;					/* 虚拟地址 */
	mem_addr32 *virtual_svr_address;					/* SVR寄存器 */
	mem_addr32 *virtual_id_address;             		/* id寄存器 */
	mem_addr32 *virtual_version_address;        		/* 版本寄存器 */
	mem_addr32 *virtual_ldr_address;        			/* 逻辑目标寄存器LDR */
	mem_addr32 *virtual_dfr_address;        			/* 目标格式寄存器DFR */
	mem_addr32 *virtual_tpr_address;        			/* TPR寄存器 */
	mem_addr32 *virtual_ppr_address;        			/* PPR寄存器 */
	mem_addr32 *virtual_eoi_address;					/* EOI寄存器 */
	mem_addr32 *virtual_lvt_cmci_address;				/* LVT CMCI寄存器 */
	mem_addr32 *virtual_icr_low_address;				/* 中断命令寄存器ICR(0-31bit) */
	mem_addr32 *virtual_icr_high_address;				/* 中断命令寄存器ICR(32-63bit) */
	mem_addr32 *virtual_lvt_timer_address;				/* LVT 定时器寄存器 */
	mem_addr32 *virtual_lvt_temperature_address;		/* LVT 温度寄存器 */
	mem_addr32 *virtual_lvt_perfor_address;     	   	/* LVT 性能监控寄存器 */
	mem_addr32 *virtual_lvt_LINT0_address;     	   		/* LVT LINT0寄存器 */
	mem_addr32 *virtual_lvt_LINT1_address;     	   		/* LVT LINT1寄存器 */
	mem_addr32 *virtual_lvt_err_address;				/* LVT 错误寄存器 */
} local_APIC_map;

struct IOAPIC_map {
	/* IO APIC相关地址映射结构 */
	mem_addr32 physical_address;
	mem_addr8 *virtual_index_address;
	mem_addr32 *virtual_data_address;
	mem_addr32 *virtual_EOI_address;
}ioapic_map;

unsigned long ioapic_rte_read(unsigned char index);
void ioapic_rte_write(unsigned char index,unsigned long value);

void APIC_init();				/* 初始化APIC(local APIC, IO APIC) */
void local_APIC_init();			/* 初始化local APIC */
void IOAPIC_pagetable_remap();	/* 初始化IOAPIC内存映射 */
