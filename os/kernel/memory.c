...
unsigned long get_page_attribute(struct Page * page) {
	if(page == NULL) {
		color_printk(RED,BLACK,"[memory]get_page_attribute() ERROR: page == NULL\n");
		return 0;
	}
	else
...
unsigned long set_page_attribute(struct Page * page,unsigned long flags) {
	if(page == NULL) {
		color_printk(RED,BLACK,"[memory]set_page_attribute() ERROR: page == NULL\n");
		return 0;
	} else {
		page->attribute = flags;
		return 1;
...
void init_memory() {
	...
	color_printk(BLUE,BLACK,"[memory]Display Physics Address MAP,Type(1:RAM,2:ROM or Reserved,3:ACPI Reclaim Memory,4:ACPI NVS Memory,Others:Undefine)\n");
	...
	color_printk(ORANGE,BLACK,"[memory]OS Can Used Total RAM:%#018lx\n",TotalMem);
	TotalMem = 0;
	...
	color_printk(ORANGE,BLACK,"[memory]OS Can Used Total 2M PAGEs:%#018lx=%018ld\n",TotalMem,TotalMem);
	...
	color_printk(ORANGE,BLACK,"[memory]bits_map:%#018lx,bits_size:%#018lx,bits_length:%#018lx\n",memory_management_struct.bits_map,memory_management_struct.bits_size,memory_management_struct.bits_length);
	color_printk(ORANGE,BLACK,"[memory]pages_struct:%#018lx,pages_size:%#018lx,pages_length:%#018lx\n",memory_management_struct.pages_struct,memory_management_struct.pages_size,memory_management_struct.pages_length);
	color_printk(ORANGE,BLACK,"[memory]zones_struct:%#018lx,zones_size:%#018lx,zones_length:%#018lx\n",memory_management_struct.zones_struct,memory_management_struct.zones_size,memory_management_struct.zones_length);
	...
	for(i = 0;i < memory_management_struct.zones_size;i++) {
		...
		color_printk(ORANGE,BLACK,"[memory]zone_start_address:%#018lx,zone_end_address:%#018lx,zone_length:%#018lx,pages_group:%#018lx,pages_length:%#018lx\n",z->zone_start_address,z->zone_end_address,z->zone_length,z->pages_group,z->pages_length);

		if(z->zone_start_address >= 0x100000000 && !ZONE_UNMAPED_INDEX)
			ZONE_UNMAPED_INDEX = i;
	}
	
	color_printk(ORANGE,BLACK,"[memory]ZONE_DMA_INDEX:%d\tZONE_NORMAL_INDEX:%d\tZONE_UNMAPED_INDEX:%d\n",ZONE_DMA_INDEX,ZONE_NORMAL_INDEX,ZONE_UNMAPED_INDEX);
	...
	color_printk(ORANGE,BLACK,"[memory]start_code:%#018lx,end_code:%#018lx,end_data:%#018lx,end_brk:%#018lx,end_of_struct:%#018lx\n",memory_management_struct.start_code,memory_management_struct.end_code,memory_management_struct.end_data,memory_management_struct.end_brk, memory_management_struct.end_of_struct);
	...
	color_printk(INDIGO,BLACK,"[memory]Global_CR3\t:%#018lx\n",Global_CR3);
...
struct Page * alloc_pages(int zone_select,int number,unsigned long page_flags) {
	...
	if(number >= 64 || number <= 0) {
		color_printk(RED,BLACK,"[memory]alloc_pages() ERROR: number is invalid\n");
		return NULL;		
	}
	...
	switch(zone_select) {
		...
		default:
			color_printk(RED,BLACK,"[memory]alloc_pages() ERROR: zone_select index is invalid\n");
			return NULL;
			break;
	}
	...
	color_printk(RED,BLACK,"[memory]alloc_pages() ERROR: no page can alloc\n");
	return NULL;
...
void free_pages(struct Page * page,int number) {	
	...
	if(page == NULL) {
		color_printk(RED,BLACK,"[memory]free_pages() ERROR: page is invalid\n");
		return ;
	}	

	if(number >= 64 || number <= 0) {
		color_printk(RED,BLACK,"[memory]free_pages() ERROR: number is invalid\n");
		return ;	
	}
...
void * kmalloc(unsigned long size,unsigned long gfp_flages) {
	...
	if(size > 1048576) {
		color_printk(RED,BLACK,"[memory]kmalloc() ERROR: kmalloc size too long:%08d\n",size);
		return NULL;
	}
	for(i = 0;i < 16;i++)
	...
	if(kmalloc_cache_size[i].total_free != 0) {
		...
		if(slab == NULL) {
			color_printk(BLUE,BLACK,"[memory]kmalloc()->kmalloc_create()=>slab == NULL\n");
			return NULL;
		}
		
		kmalloc_cache_size[i].total_free += slab->color_count;
		color_printk(BLUE,BLACK,"[memory]kmalloc()->kmalloc_create()<=size:%#010x\n",kmalloc_cache_size[i].size);///////
	...
	color_printk(BLUE,BLACK,"[memory]kmalloc() ERROR: no memory can alloc\n");
	return NULL;
}
...
struct Slab * kmalloc_create(unsigned long size) {
	...
	if(page == NULL) {
		color_printk(RED,BLACK,"[memory]kmalloc_create()->alloc_pages()=>page == NULL\n");
		return NULL;
	}
	...
	switch(size) {
		...
		default:
			...
			color_printk(RED,BLACK,"[memory]kmalloc_create() ERROR: wrong size:%08d\n",size);
			free_pages(page,1);
...
unsigned long kfree(void * address) {
	...
	color_printk(RED,BLACK,"[memory]kfree() ERROR: can`t free memory\n");
	return 0;
}
...
struct Slab_cache * slab_create(unsigned long size,void *(* constructor)(void * Vaddress,unsigned long arg),void *(* destructor)(void * Vaddress,unsigned long arg),unsigned long arg) {
	...
	if(slab_cache == NULL) {
		color_printk(RED,BLACK,"[memory]slab_create()->kmalloc()=>slab_cache == NULL\n");
		return NULL;
	}
	...
	if(slab_cache->cache_pool == NULL) {
		color_printk(RED,BLACK,"[memory]slab_create()->kmalloc()=>slab_cache->cache_pool == NULL\n");
		kfree(slab_cache);
		return NULL;
	}
	...
	if(slab_cache->cache_pool->page == NULL) {
		color_printk(RED,BLACK,"[memory]slab_create()->alloc_pages()=>slab_cache->cache_pool->page == NULL\n");
	...
	if(slab_cache->cache_pool->color_map == NULL) {
		color_printk(RED,BLACK,"[memory]slab_create()->kmalloc()=>slab_cache->cache_pool->color_map == NULL\n");
...
unsigned long slab_destroy(struct Slab_cache * slab_cache) {
	...
	if(slab_cache->total_using != 0) {
		color_printk(RED,BLACK,"[memory]slab_cache->total_using != 0\n");
		return 0;
	}
...
void * slab_malloc(struct Slab_cache * slab_cache,unsigned long arg) {
	...
	if(slab_cache->total_free == 0) {
		...
		if(tmp_slab == NULL) {
			color_printk(RED,BLACK,"[memory]slab_malloc()->kmalloc()=>tmp_slab == NULL\n");
			return NULL;
		}
		...
		if(tmp_slab->page == NULL) {
			color_printk(RED,BLACK,"[memory]slab_malloc()->alloc_pages()=>tmp_slab->page == NULL\n");
			kfree(tmp_slab);
			return NULL;
		}
		...
		if(tmp_slab->color_map == NULL) {
			color_printk(RED,BLACK,"[memory]slab_malloc()->kmalloc()=>tmp_slab->color_map == NULL\n");
			free_pages(tmp_slab->page,1);
			kfree(tmp_slab);
			return NULL;
		}
	...
	color_printk(RED,BLACK,"[memory]slab_malloc() ERROR: can`t alloc\n");
	if(tmp_slab != NULL) {
		list_del(&tmp_slab->list);
		kfree(tmp_slab->color_map);
		page_clean(tmp_slab->page);
...
unsigned long slab_free(struct Slab_cache * slab_cache,void * address,unsigned long arg) {
	...
	color_printk(RED,BLACK,"[memory]slab_free() ERROR: address not in slab\n");
...
unsigned long slab_init() {
	...
	color_printk(ORANGE,BLACK,"[memory]2.memory_management_struct.bits_map:%#018lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d\n",*memory_management_struct.bits_map,memory_management_struct.zones_struct->page_using_count,memory_management_struct.zones_struct->page_free_count);
	...
	color_printk(ORANGE,BLACK,"[memory]3.memory_management_struct.bits_map:%#018lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d\n",*memory_management_struct.bits_map,memory_management_struct.zones_struct->page_using_count,memory_management_struct.zones_struct->page_free_count);
	color_printk(ORANGE,BLACK,"[memory]start_code:%#018lx,end_code:%#018lx,end_data:%#018lx,end_brk:%#018lx,end_of_struct:%#018lx\n",memory_management_struct.start_code,memory_management_struct.end_code,memory_management_struct.end_data,memory_management_struct.end_brk, memory_management_struct.end_of_struct);
