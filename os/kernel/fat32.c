...
struct FAT32_BootSector fat32_bootsector;	/* 启动扇区 */
struct FAT32_FSInfo fat32_fsinfo;			/* FSInfo结构 */

unsigned long FirstDataSector = 0;     /* 数据区起始扇区号 */
unsigned long BytesPerClus = 0;        /* 每簇字节数 */
unsigned long FirstFAT1Sector = 0;     /* FAT1表起始扇区号 */
unsigned long FirstFAT2Sector = 0;     /* FAT2表起始扇区号 */
...
next_cluster:
	...
	color_printk(BLUE,WHITE,"lookup cluster:%#010x,sector:%#018lx\n",cluster,sector);
	if(!IDE_device_operation.transfer(ATA_READ_CMD,sector,fat32_bootsector.BPB_SecPerClus,(unsigned char *)buf)) {
		color_printk(RED,WHITE,"FAT32 FS(lookup) read disk ERROR!!!!!!!!!!\n");
		kfree(buf);
		return NULL;
	}
