...
unsigned long do_execve(struct pt_regs * regs) {
	...
	regs->ds = 0;
	regs->es = 0;
	color_printk(RED,BLACK,"[mtask]do_execve task is running\n");
	...
	memcpy(user_level_function,(void *)0x800000,1024);	/* 将user_level_function程序复制到线性地址0x800000处 */
...
unsigned long init(unsigned long arg) {
	...
	color_printk(RED,BLACK,"[mtask]init task is running,arg:%#018lx\n",arg);
...
unsigned long do_fork(struct pt_regs * regs, unsigned long clone_flags, unsigned long stack_start, unsigned long stack_size) {
	...
	color_printk(WHITE,BLACK,"[mtask]alloc_pages,bitmap:%#018lx\n",*memory_management_struct.bits_map);
	...
	color_printk(WHITE,BLACK,"[mtask]struct task_struct address:%#018lx\n",(unsigned long)tsk);
...
void task_init() {
	...
	/* 
	* _stack_start出处位于head.S:
	* ENTRY(_stack_start)
	* 	.quad	init_task_union + 32768
	* 其记录了系统第一个进程的内核层栈基地址(参考Kernel.lds)
	*/
	
	wrmsr(0x174,KERNEL_CS);							/* 设置0特权级代码段选择子到IA32_SYSENTER_CS，该寄存器的低16位是R0的代码段的选择子。该值同时用来计算R0的堆栈的选择子。 */
	wrmsr(0x175,current->thread->rsp0);             /* IA32_SYSENTER_ESP寄存器的值会被载入到RSP中，指向第一条指令 */
	wrmsr(0x176,(unsigned long)system_call);        /* IA32_SYSENTER_EIP寄存器的值会被载入到RIP中，指向栈堆 */
