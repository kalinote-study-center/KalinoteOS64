...
unsigned long do_exit(unsigned long code) {
	color_printk(RED,BLACK,"[mtask]exit task is running,arg:%#018lx\n",code);
	while(1);
}
...
inline void __switch_to(struct task_struct *prev,struct task_struct *next) {
	...
	color_printk(WHITE,BLACK,"[mtask]prev->thread->rsp0:%#018lx\t",prev->thread->rsp0);
	color_printk(WHITE,BLACK,"[mtask]prev->thread->rsp :%#018lx\n",prev->thread->rsp);
	color_printk(WHITE,BLACK,"[mtask]next->thread->rsp0:%#018lx\t",next->thread->rsp0);
	color_printk(WHITE,BLACK,"[mtask]next->thread->rsp :%#018lx\n",next->thread->rsp);
}
