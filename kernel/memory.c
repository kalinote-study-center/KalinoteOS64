...
unsigned long get_page_attribute(struct Page * page) {
	if(page == NULL) {
		color_printk(COL_RED,COL_BLACK,"get_page_attribute() ERROR: page == NULL\n");
		return 0;
	}
	else
...
unsigned long set_page_attribute(struct Page * page,unsigned long flags) {
	if(page == NULL) {
		color_printk(COL_RED,COL_BLACK,"set_page_attribute() ERROR: page == NULL\n");
		return 0;
	}
	else
	{
...
void init_memory(void) {
	...
	color_printk(COL_BLUE,COL_BLACK,"Display Physics Address MAP,Type(1:RAM,2:ROM or Reserved,3:ACPI Reclaim Memory,4:ACPI NVS Memory,Others:Undefine)\n");
	p = (struct E820 *)0xffff800000007e00;

	for(i = 0;i < 32;i++) {
		if(p->type == 1)
		color_printk(COL_ORANGE,COL_BLACK,"Address:%#018lx\tLength:%#018lx\tType:%#010x\n",p->address,p->length,p->type);
		unsigned long tmp = 0;
		if(p->type == 1)
			TotalMem +=  p->length;
	...
	color_printk(COL_ORANGE,COL_BLACK,"Total RAM:%#018lx\n",TotalMem);
	...
	/* 初始化地址0到页面结构0；因为memory_management_struct.e820[0].type != 1 */
	...
	color_printk(COL_ORANGE,COL_BLACK,"bits_map:%#018lx,bits_size:%#018lx,bits_length:%#018lx\n",memory_management_struct.bits_map,memory_management_struct.bits_size,memory_management_struct.bits_length);
	color_printk(COL_ORANGE,COL_BLACK,"pages_struct:%#018lx,pages_size:%#018lx,pages_length:%#018lx\n",memory_management_struct.pages_struct,memory_management_struct.pages_size,memory_management_struct.pages_length);
	color_printk(COL_ORANGE,COL_BLACK,"zones_struct:%#018lx,zones_size:%#018lx,zones_length:%#018lx\n",memory_management_struct.zones_struct,memory_management_struct.zones_size,memory_management_struct.zones_length);
	...
	for(i = 0;i < memory_management_struct.zones_size;i++) {
		...
		color_printk(COL_ORANGE,COL_BLACK,"zone_start_address:%#018lx,zone_end_address:%#018lx,zone_length:%#018lx,pages_group:%#018lx,pages_length:%#018lx\n",z->zone_start_address,z->zone_end_address,z->zone_length,z->pages_group,z->pages_length);

		if(z->zone_start_address >= 0x100000000 && !ZONE_UNMAPED_INDEX)
			ZONE_UNMAPED_INDEX = i;
	}
	
	color_printk(COL_ORANGE,COL_BLACK,"ZONE_DMA_INDEX:%d\tZONE_NORMAL_INDEX:%d\tZONE_UNMAPED_INDEX:%d\n",ZONE_DMA_INDEX,ZONE_NORMAL_INDEX,ZONE_UNMAPED_INDEX);
	...
	color_printk(COL_ORANGE,COL_BLACK,"start_code:%#018lx,end_code:%#018lx,end_data:%#018lx,end_brk:%#018lx,end_of_struct:%#018lx\n",memory_management_struct.start_code,memory_management_struct.end_code,memory_management_struct.end_data,memory_management_struct.end_brk, memory_management_struct.end_of_struct);
...
struct Page * alloc_pages(int zone_select,int number,unsigned long page_flags) {
	...
	if(number >= 64 || number <= 0) {
		color_printk(COL_RED,COL_BLACK,"alloc_pages() ERROR: number is invalid\n");
		return NULL;		
	}
	...
	switch(zone_select) {
		...
		default:
			color_printk(COL_RED,COL_BLACK,"alloc_pages() ERROR: zone_select index is invalid\n");
			return NULL;
			break;
	}
	...
	color_printk(COL_RED,COL_BLACK,"alloc_pages() ERROR: no page can alloc\n");
	return NULL;
...
void free_pages(struct Page * page,int number) {	
	...
	if(page == NULL) {
		color_printk(COL_RED,COL_BLACK,"free_pages() ERROR: page is invalid\n");
		return ;
	}	

	if(number >= 64 || number <= 0) {
		color_printk(COL_RED,COL_BLACK,"free_pages() ERROR: number is invalid\n");
		return ;	
	}
...
void * kmalloc(unsigned long size,unsigned long gfp_flages) {
	...
	if(size > 1048576) {
		color_printk(COL_RED,COL_BLACK,"kmalloc() ERROR: kmalloc size too long:%08d\n",size);
		return NULL;
	}
	for(i = 0;i < 16;i++)
	...
	if(kmalloc_cache_size[i].total_free != 0) {
		...
		if(slab == NULL) {
			color_printk(COL_BLUE,COL_BLACK,"kmalloc()->kmalloc_create()=>slab == NULL\n");
			return NULL;
		}
		
		kmalloc_cache_size[i].total_free += slab->color_count;

		color_printk(COL_BLUE,COL_BLACK,"kmalloc()->kmalloc_create()<=size:%#010x\n",kmalloc_cache_size[i].size);///////
	...
	color_printk(COL_BLUE,COL_BLACK,"kmalloc() ERROR: no memory can alloc\n");
	return NULL;
}
...
struct Slab * kmalloc_create(unsigned long size) {
	...
	if(page == NULL) {
		color_printk(COL_RED,COL_BLACK,"kmalloc_create()->alloc_pages()=>page == NULL\n");
		return NULL;
	}
	...
	switch(size) {
		...
		default:
			...
			color_printk(COL_RED,COL_BLACK,"kmalloc_create() ERROR: wrong size:%08d\n",size);
			free_pages(page,1);
...
unsigned long kfree(void * address) {
	...
	color_printk(COL_RED,COL_BLACK,"kfree() ERROR: can`t free memory\n");
...
struct Slab_cache * slab_create(unsigned long size,void *(* constructor)(void * Vaddress,unsigned long arg),void *(* destructor)(void * Vaddress,unsigned long arg),unsigned long arg) {
	...
	if(slab_cache == NULL) {
		color_printk(COL_RED,COL_BLACK,"slab_create()->kmalloc()=>slab_cache == NULL\n");
		return NULL;
	}
	...
	if(slab_cache->cache_pool == NULL) {
		color_printk(COL_RED,COL_BLACK,"slab_create()->kmalloc()=>slab_cache->cache_pool == NULL\n");
		kfree(slab_cache);
		return NULL;
	}
	...
	if(slab_cache->cache_pool->page == NULL) {
		color_printk(COL_RED,COL_BLACK,"slab_create()->alloc_pages()=>slab_cache->cache_pool->page == NULL\n");
	...
	if(slab_cache->cache_pool->color_map == NULL) {
		color_printk(COL_RED,COL_BLACK,"slab_create()->kmalloc()=>slab_cache->cache_pool->color_map == NULL\n");
...
unsigned long slab_destroy(struct Slab_cache * slab_cache) {
	...
	if(slab_cache->total_using != 0) {
		color_printk(COL_RED,COL_BLACK,"slab_cache->total_using != 0\n");
		return 0;
	}
...
void * slab_malloc(struct Slab_cache * slab_cache,unsigned long arg) {
	...
	if(slab_cache->total_free == 0) {
		...
		if(tmp_slab == NULL) {
			color_printk(COL_RED,COL_BLACK,"slab_malloc()->kmalloc()=>tmp_slab == NULL\n");
			return NULL;
		}
		...
		if(tmp_slab->page == NULL) {
			color_printk(COL_RED,COL_BLACK,"slab_malloc()->alloc_pages()=>tmp_slab->page == NULL\n");
			kfree(tmp_slab);
			return NULL;
		}
		...
		if(tmp_slab->color_map == NULL) {
			color_printk(COL_RED,COL_BLACK,"slab_malloc()->kmalloc()=>tmp_slab->color_map == NULL\n");
			free_pages(tmp_slab->page,1);
			kfree(tmp_slab);
			return NULL;
		}
	...
	color_printk(COL_RED,COL_BLACK,"slab_malloc() ERROR: can`t alloc\n");
	if(tmp_slab != NULL) {
		list_del(&tmp_slab->list);
		kfree(tmp_slab->color_map);
		page_clean(tmp_slab->page);
...
unsigned long slab_free(struct Slab_cache * slab_cache,void * address,unsigned long arg) {
	...
	color_printk(COL_RED,COL_BLACK,"slab_free() ERROR: address not in slab\n");
...
unsigned long slab_init() {
	...
	color_printk(COL_ORANGE,COL_BLACK,"2.memory_management_struct.bits_map:%#018lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d\n",*memory_management_struct.bits_map,memory_management_struct.zones_struct->page_using_count,memory_management_struct.zones_struct->page_free_count);
	...
	color_printk(COL_ORANGE,COL_BLACK,"3.memory_management_struct.bits_map:%#018lx\tzone_struct->page_using_count:%d\tzone_struct->page_free_count:%d\n",*memory_management_struct.bits_map,memory_management_struct.zones_struct->page_using_count,memory_management_struct.zones_struct->page_free_count);
...
void pagetable_init() {
	...
	color_printk(COL_YELLOW,COL_BLACK,"1:%#018lx,%#018lx\t\t\n",(unsigned long)tmp,*tmp);

	tmp = Phy_To_Virt(*tmp & (~0xfffUL));

	color_printk(COL_YELLOW,COL_BLACK,"2:%#018lx,%#018lx\t\t\n",(unsigned long)tmp,*tmp);

	tmp = Phy_To_Virt(*tmp & (~0xfffUL));

	color_printk(COL_YELLOW,COL_BLACK,"3:%#018lx,%#018lx\t\t\n",(unsigned long)tmp,*tmp);
	...
	for(i = 0;i < memory_management_struct.zones_size;i++) {
		...
		for(j = 0;j < z->pages_length ; j++,p++) {
			...
			if(j % 50 == 0)
				color_printk(COL_GREEN,COL_BLACK,"@:%#018lx,%#018lx\t\n",(unsigned long)tmp,*tmp);
		}
	}
