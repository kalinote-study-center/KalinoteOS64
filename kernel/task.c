...
unsigned long do_execve(struct pt_regs * regs) {
	...
	regs->ds = 0;
	regs->es = 0;
	color_printk(COL_RED,COL_BLACK,"do_execve task is running\n");
...
unsigned long init(unsigned long arg) {
	...
	color_printk(COL_RED,COL_BLACK,"init task is running,arg:%#018lx\n",arg);
...
unsigned long do_fork(struct pt_regs * regs, unsigned long clone_flags, unsigned long stack_start, unsigned long stack_size) {
	...
	color_printk(COL_WHITE,COL_BLACK,"struct task_struct address:%#018lx\n",(unsigned long)tsk);
...
void __switch_to(struct task_struct *prev,struct task_struct *next) {
	...
	color_printk(COL_WHITE,COL_BLACK,"prev->thread->rsp0:%#018lx\n",prev->thread->rsp0);
	color_printk(COL_WHITE,COL_BLACK,"next->thread->rsp0:%#018lx\n",next->thread->rsp0);
}
