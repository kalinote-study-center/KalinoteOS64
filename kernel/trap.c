...
void do_invalid_TSS(struct pt_regs * regs,unsigned long error_code) {
	...
	if(error_code & 0x02)
		...
		color_printk(COL_RED,COL_BLACK,"Refers to a descriptor in the GDT or the current LDT;\n");

	if((error_code & 0x02) == 0)
		if(error_code & 0x04)
			color_printk(COL_RED,COL_BLACK,"Refers to a segment or gate descriptor in the LDT;\n");
		else
			color_printk(COL_RED,COL_BLACK,"Refers to a descriptor in the current GDT;\n");

	color_printk(COL_RED,COL_BLACK,"Segment Selector Index:%#010x\n",error_code & 0xfff8);
...
void do_page_fault(struct pt_regs * regs,unsigned long error_code) {
	...
	if(error_code & 0x02)
		color_printk(COL_RED,COL_BLACK,"Write Cause Fault,\t");
	else
		color_printk(COL_RED,COL_BLACK,"Read Cause Fault,\t");

	if(error_code & 0x04)
		color_printk(COL_RED,COL_BLACK,"Fault in user(3)\t");
	else
		color_printk(COL_RED,COL_BLACK,"Fault in supervisor(0,1,2)\t");

	if(error_code & 0x08)
		color_printk(COL_RED,COL_BLACK,",Reserved Bit Cause Fault\t");

	if(error_code & 0x10)
		color_printk(COL_RED,COL_BLACK,",Instruction fetch Cause Fault");

	color_printk(COL_RED,COL_BLACK,"\n");

	color_printk(COL_RED,COL_BLACK,"CR2:%#018lx\n",cr2);
