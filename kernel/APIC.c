...
void IOAPIC_pagetable_remap() {
	...
	color_printk(COL_BLUE,COL_BLACK,"3:%#018lx\t%#018lx\n",(unsigned long)tmp,(unsigned long)*tmp);

	color_printk(COL_BLUE,COL_BLACK,"ioapic_map.physical_address:%#010x\t\t\n",ioapic_map.physical_address);
	color_printk(COL_BLUE,COL_BLACK,"ioapic_map.virtual_address:%#018lx\t\t\n",(unsigned long)ioapic_map.virtual_index_address);
...
void Local_APIC_init() {
	...
	color_printk(COL_WHITE,COL_BLACK,"CPUID\t01,eax:%#010x,ebx:%#010x,ecx:%#010x,edx:%#010x\n",a,b,c,d);

	if((1<<9) & d)
		color_printk(COL_WHITE,COL_BLACK,"HW support APIC&xAPIC\t");
	else
		color_printk(COL_WHITE,COL_BLACK,"HW NO support APIC&xAPIC\t");
	
	if((1<<21) & c)
		color_printk(COL_WHITE,COL_BLACK,"HW support x2APIC\n");
	else
		color_printk(COL_WHITE,COL_BLACK,"HW NO support x2APIC\n");
	...
	color_printk(COL_WHITE,COL_BLACK,"eax:%#010x,edx:%#010x\t",x,y);
	
	if(x&0xc00)
		color_printk(COL_WHITE,COL_BLACK,"xAPIC & x2APIC enabled\n");
	...
	if(x&0x100)
		color_printk(COL_WHITE,COL_BLACK,"SVR[8] enabled\n");
	if(x&0x1000)
		color_printk(COL_WHITE,COL_BLACK,"SVR[12] enabled\n");
	...
	color_printk(COL_WHITE,COL_BLACK,"eax:%#010x,edx:%#010x\tx2APIC ID:%#010x\n",x,y,x);
	...
	color_printk(COL_WHITE,COL_BLACK,"local APIC Version:%#010x,Max LVT Entry:%#010x,SVR(Suppress EOI Broadcast):%#04x\t",x & 0xff,(x >> 16 & 0xff) + 1,x >> 24 & 0x1);

	if((x & 0xff) < 0x10)
		color_printk(COL_WHITE,COL_BLACK,"82489DX discrete APIC\n");
	...
	color_printk(COL_GREEN,COL_BLACK,"Mask ALL LVT\n");
	...
	color_printk(COL_GREEN,COL_BLACK,"Set LVT TPR:%#010x\t",x);
	...
	color_printk(COL_GREEN,COL_BLACK,"Set LVT PPR:%#010x\n",x);
...
void APIC_IOAPIC_init() {
	...
	//mask 8259A
	color_printk(COL_GREEN,COL_BLACK,"MASK 8259A\n");
	io_out8(0x21,0xff);
	io_out8(0xa1,0xff);
	//enable IMCR
	io_out8(0x22,0x70);
